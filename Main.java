/*	Program name:	Lab 03 Employee Schedules
	Programmer:		Marcus Ross
	Date Due:		24 Feb, 2014
	Description:	This program allows the user to manage employee schedules. It manipulates one list at any time representing one person's schedule comprised of 100 schedule entries (because scheduling someone that many days in advance would just be irresponsible), each consisting of a begin time, an end time, a location, and a comment.	*/
...
public class Main {
	...
	public static void MainMenu(List schedule, Frame frame) { // go to main menu
		...
		btnSave.addActionListener(new SaveMenu(schedule, frame));
	...
	public static void LoadFile(List schedule, Frame frame, Label errorL, String fileN) { // called from "Load a schedule" menu
		...
		try {
			while(fileRead.hasNext()) { // read four lines belonging to each entry
				...
				if(!schedule.add(entry)) { // add entry to list
					errorL.setText("Number of entries has reached capacity");
					return;
				}
			}
		...
		MainMenu(schedule, frame);
	}

	public static void DoSaveMenu(List schedule, Frame frame) { // called to load "Save a schedule" menu
		// frame methods
		frame.removeAll();
		...
		// component methods
		errorL.setForeground(Color.red);
		btnOk.addActionListener(new SaveOk(schedule, frame, errorL, fileNF));
		btnCancel.addActionListener(new SaveOk(schedule, frame, errorL, fileNF));
	...
	public static void SaveFile(List schedule, Frame frame, Label errorL, String fileN) { // called from "Save a schedule" menu
		PrintWriter fileWrite;

		try {
			fileWrite = new PrintWriter(new BufferedWriter(new FileWriter(new File(fileN), false)));
		} catch(FileNotFoundException f) {
			errorL.setText("Inventory file not found");
			return;
		} catch(IOException i) {
			errorL.setText("Inventory file i/o error");
			return;
		}

		schedule.reset();

		do { // output 4 lines to file per entry
			fileWrite.printf("%s\n%d\n%d\n%s\n", schedule.retrieve().getLocation(), schedule.retrieve().getTimeBegin().getTime(), schedule.retrieve().getTimeEnd().getTime(), schedule.retrieve().getComment());
		} while (schedule.getNext() && !schedule.atEnd()); // traverse list until end reached
	...
	public static void DispRange(List schedule, Frame frame, Date dateLow, Date dateHigh) { // called from "Display a range" menu
		...
		} while(schedule.getNext() && !schedule.atEnd() && !schedule.isEmpty()); // traverse list until at end
...
class SaveMenu implements ActionListener { // used by "Save a schedule" button
	...
	public SaveMenu(List schedule, Frame frame) {
		this.schedule = schedule;	this.frame = frame;
	}

	public void actionPerformed(ActionEvent e) {
		Main.DoSaveMenu(schedule, frame);
	}
}
...
class SaveOk implements ActionListener { // used when pressing "OK" or "Cancel" from "Save a schedule" menu
	...
	public SaveOk(List schedule, Frame frame, Label errorL, TextField fileNF) {
		this.schedule = schedule;	this.frame = frame;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("OK"))
			Main.SaveFile(schedule, frame, errorL, fileN); // save the schedule if "OK" pressed
		else
			Main.MainMenu(schedule, frame); // load main menu if "Cancel" pressed
	}
}
